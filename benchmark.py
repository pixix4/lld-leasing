...
def main():
    ...
    print(grouped)
    f = open(os.path.join(dir, 'benchmark-grouped.txt'), "w")
    f.write(str(grouped))
    f.close()
    ...
    fig, ax = plt.subplots(1, 2, figsize=(10, 5))
    fig = grouped['granted_count'].unstack().plot(kind='bar', ax=ax[0])
    fig.set_xlabel('Number of concurrent clients')
    fig.set_ylabel('Avg count')
    fig.legend()

    fig = grouped['granted_count'].unstack().plot(kind='bar', ax=ax[1])
    fig.set_xlabel('Number of concurrent clients ($90^{th}$ percentile)')
    fig.set_ylabel('Avg count')
    fig.legend()
    fig.set_ylim(0, grouped['granted_count'].quantile(0.9))

    # plt.semilogy()
    plt.tight_layout()
    plt.savefig(os.path.join(dir, 'response-count-granted.png'))
    plt.clf()
